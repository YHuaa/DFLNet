...
from utils.utils import pyContourf_two, figure, figureOne
import time
import h5py
...
# test_real_data_path = './data/sound_source_data_fixed_distance/RealTest/One_1.mat'
dataDir = './One_16K_z6/'
results_dir = './img_results/TestTime/'
# ckpt = './models/06-17-12-04/last.pth'
ckpt = './models/07-08-09-20/last.pth'
# ckpt = './models/06-17-12-04/ckpt_epoch_50.pth'
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        # 是否加噪声测试
        if add_Noise:
            raw_sound_data = addnoise(raw_sound_data, snr=-20)

        print(path)
        sample_name = path.split('/')[-1].split('.')[0]
        # print('data_shape:', data.shape)
        # # data.shape = [9600, 64]
        # raw_sound_data = scio.loadmat(test_real_data_path)['a'][:9600]
        ...
        # for idx, (ATA, ATb, DAS_results, label, _, sample_name) in enumerate(test_dataloader):
            ...
            # scio.savemat('Four_1_B.mat',{'B':np_output})
            # scio.savemat('./One_16K_z6_mat/{:}.mat'.format(sample_name), {'B':np_output})
            zero = np.zeros((41, 41))
            figure(np_output, DAS_result, results_dir, sample_name)
            # figureOne('DFLNet', np_output)
    print(sum_time / (len(list)-1))
